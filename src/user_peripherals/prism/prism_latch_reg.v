...
`ifndef IHP_OR2_1
`ifdef IHP_SG13G2
`define IHP_OR2_1 sg13g2_or2_1
`else
`define IHP_OR2_1 sg13cmos5l_or2_1
`endif
`endif
`ifndef IHP_OR2_2
`ifdef IHP_SG13G2
`define IHP_OR2_2 sg13g2_or2_2
`else
`define IHP_OR2_2 sg13cmos5l_or2_2
`endif
`endif
...
`else
    ...
    genvar b;
    generate
        (* keep = 1 *) `IHP_AND2_1 gate_and (.A(enable), .B(wr), .X(pre_reset));
        if (WIDTH < 6) begin : GEN_OR_X1
            (* keep = 1 *) `IHP_OR2_1 gate_or (.A(pre_reset), .B(~rst_n), .X(gate));
        end else if (WIDTH < 12) begin : GEN_OR_X2
            (* keep = 1 *) `IHP_OR2_2 gate_or (.A(pre_reset), .B(~rst_n), .X(gate));
        end else begin : GEN_OR_X4
            (* keep = 1 *) `IHP_OR2_2 gate_or (.A(pre_reset), .B(~rst_n), .X(gate));
        end
        ...
        for (b = 0; b < WIDTH; b = b + 1)
        begin : gen_prism_bit
            (* keep = 1 *) `IHP_DLHQ_1 prism_cfg_bit
            (
                .D    (data_in[b]),
                .GATE (gate),
                .Q    (latch_data[b])
