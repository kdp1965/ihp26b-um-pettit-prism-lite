...
// Both IHP libraries (sg13cmos5l, sg13g2) have the same latch / clock-gate cells
...
`ifndef IHP_DLHQ_1
`ifdef IHP_SG13G2
`define IHP_DLHQ_1 sg13g2_dlhq_1
`else
`define IHP_DLHQ_1 sg13cmos5l_dlhq_1
`endif
`endif
`ifndef IHP_INV_1
`ifdef IHP_SG13G2
`define IHP_INV_1 sg13g2_inv_1
`else
`define IHP_INV_1 sg13cmos5l_inv_1
`endif
`endif
`ifndef IHP_LGCP_1
`ifdef IHP_SG13G2
`define IHP_LGCP_1 sg13g2_lgcp_1
`else
`define IHP_LGCP_1 sg13cmos5l_lgcp_1
`endif
`endif
...
`elsif SCL_IHP
    ...
    // Lint for PDK cells expects power pins, so disable the warning
    /* verilator lint_off PINMISSING */
    `IHP_INV_1 CLKINV(.Y(clk_b), .A(clk));

    /* verilator lint_off GENUNNAMED */
    generate
        if (WIDTH <= 6)
            `IHP_LGCP_1 CG( .CLK(clk_b), .GCLK(gated_clk), .GATE(wen) );
        else if (WIDTH <= 12)
            `IHP_LGCP_1 CG( .CLK(clk_b), .GCLK(gated_clk), .GATE(wen) );
        else
            `IHP_LGCP_1 CG( .CLK(clk_b), .GCLK(gated_clk), .GATE(wen) );
    ...
    generate
        for (i = 0; i < WIDTH; i = i+1) begin : gen_latch
            `IHP_DLHQ_1 state (.Q(data_out[i]), .D(data_in[i]), .GATE(gated_clk) );
        end
    endgenerate
    /* verilator lint_on PINMISSING */
`else
...
`elsif SCL_IHP
    ...
    generate
        if (WIDTH <= 6)
            `IHP_LGCP_1 CG( .CLK(clk), .GCLK(gated_clk), .GATE(wen) );
        else if (WIDTH <= 12)
            `IHP_LGCP_1 CG( .CLK(clk), .GCLK(gated_clk), .GATE(wen) );
        else
            `IHP_LGCP_1 CG( .CLK(clk), .GCLK(gated_clk), .GATE(wen) );
